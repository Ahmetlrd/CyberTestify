...
// 3) cms_cve
// ======================================================================================
/** Yönetici Özeti'ndeki "N+ pasif sinyal" iddiasının SAYI KAYNAĞI — liste ile daima birebir. */
const FP_SIGNAL_COUNT = 6; // fingerprintSources() her dilde 6 kategori döner (aşağıdaki dizilere bak)
...
function buildCmsArea(ev: CmsEvidence, bannerCves: BannerCve[] = [], locale: string = 'tr'): Area {
  ...
  lines.push(t(`CMS/çatı ve sürüm tespiti için ana sayfa yanıtı üzerinde aşağıdaki **${FP_SIGNAL_COUNT} pasif sinyal kategorisine** bakıldı (Yönetici Özeti'ndeki “${FP_SIGNAL_COUNT}+ pasif CMS/teknoloji sinyali” ifadesi tam olarak bu listeyi kasteder; “+” her kategorinin içindeki birden çok alt sinyali belirtir):`, `Zur CMS-/Framework- und Versionserkennung wurden in der Startseiten-Antwort die folgenden **${FP_SIGNAL_COUNT} passiven Signalkategorien** betrachtet (die Angabe „${FP_SIGNAL_COUNT}+ passive CMS-/Technologiesignale" in der Managementzusammenfassung meint genau diese Liste; das „+" steht für die mehreren Untersignale je Kategorie):`, `For CMS/framework and version detection, the following **${FP_SIGNAL_COUNT} passive signal categories** in the home page response were examined (the phrase “${FP_SIGNAL_COUNT}+ passive CMS/technology signals” in the Executive Summary refers to exactly this list; the “+” denotes the multiple sub-signals within each category):`));
  lines.push('');
...
function buildUsomSection(hits: UsomHit[], catalog: UsomAdvisory[], lastSync: Date | null): string {
  ...
  // (ŞEFFAFLIK) "N bildirim tarandı" çıplak SAYI olarak bırakılmaz — hangi bildirimler olduğu
  // katalogdan AYNEN listelenir (tr no + gerçek başlık + kaynak URL). Uydurma YOK: alanlar
  // usomCatalog.ts'teki gerçek kayıtlardan gelir. Çok uzarsa ilk LIST_CAP gösterilir + kalan sayısı.
  const observableAdvisories = catalog.filter((a) => a.match.kind !== 'none');
  const notObservable = total - observable;
  if (notObservable > 0) {
    body += `_Kalan **${notObservable}** bildirim, sürümü dış yüzeyden görülemeyen ürünlere (ör. güvenlik cihazı, uç-nokta/istemci yazılımı, iç sistem) ait olduğu için bu bölümde listelenmez — bu ürünler dışarıdan eşlenemediğinden haklarında bir sonuç üretilemez._\n\n`;
  }
  if (observableAdvisories.length) {
    const LIST_CAP = 40;
    const shown = observableAdvisories.slice(0, LIST_CAP);
    const productOf = (a: UsomAdvisory): string =>
      a.match.kind === 'banner' ? a.match.product : a.match.kind === 'cms' ? a.match.cms : '—';
    body += `### Taranan dış-gözlemlenebilir bildirimler (tam liste)\n\n`;
    body += `Yukarıdaki **${observable}** sayısı soyut değildir; hedefin dış yüzeyiyle karşılaştırılan bildirimlerin tamamı aşağıdadır:\n\n`;
    body += `| TR No | Eşleme imzası (ürün/teknoloji) | Bildirim başlığı | Kaynak |\n|-----|-----|-----|-----|\n`;
    for (const a of shown) {
      body += `| ${a.tr} | ${productOf(a)} | ${a.title.replace(/\|/g, '\\|')} | [bildirim](${a.url}) |\n`;
    }
    if (observableAdvisories.length > LIST_CAP) {
      body += `\n_Yer nedeniyle ilk ${LIST_CAP} bildirim gösterilmiştir; kalan ${observableAdvisories.length - LIST_CAP} bildirim de aynı imza kurallarıyla karşılaştırılmıştır._\n`;
    }
    body += `\n`;
  }
  ...
  if (hits.length) {
    ...
    body += `✅ **Eşleşme yok:** Yukarıda tam listesi verilen **${observable}** dış-gözlemlenebilir bildirimin hiçbirinin imzası (CMS/sunucu-banner) hedefte görülmedi. Bu, hedefin bu bildirimlerden ETKİLENMEDİĞİNİ KANITLAMAZ; yalnız dış-yüzeyde ilgili ürün/sürüm sinyalinin gözlemlenmediğini gösterir.\n\n`;
  }
  return body;
}
...
export function combineReconAreas(ev: ReconEvidence, opts?: { httpOnly?: boolean; usomCatalog?: UsomAdvisory[]; usomLastSync?: Date | null }, locale: string = 'tr'): { findings: string; fixText: string } | null {
  ...
  const subRow = !ev.sub.ok ? t('⚠️ İncelenemedi (veri kaynağına ulaşılamadı — “temiz” DEĞİL)', '⚠️ Nicht prüfbar (Datenquelle nicht erreichbar — NICHT „sauber")', '⚠️ Not assessable (data source unreachable — NOT “clean”)') : ev.sub.dangling.length ? t(`⚠️ ${ev.sub.dangling.length} devralınabilir (dangling) alt domain (adları: “Subdomain Takeover Taraması” bölümü)`, `⚠️ ${ev.sub.dangling.length} übernehmbare (dangling) Subdomains (Namen: Abschnitt „Subdomain-Takeover-Prüfung")`, `⚠️ ${ev.sub.dangling.length} takeover-able (dangling) subdomains (names: “Subdomain Takeover Scan” section)`) : t(`✅ ${ev.sub.total} alt domain kaydı denendi (siteye bağlı alt adresler — isim isim liste: “Subdomain Takeover Taraması” bölümü); devralma göstergesi bulunamadı (başkasının ele geçirebileceği boşta kayıt yok)`, `✅ ${ev.sub.total} Subdomain-Einträge geprüft (mit der Site verbundene Subadressen — Liste mit Namen: Abschnitt „Subdomain-Takeover-Prüfung"); kein Übernahme-Indikator gefunden (kein verwaister Eintrag, den Dritte übernehmen könnten)`, `✅ ${ev.sub.total} subdomain records checked (sub-addresses attached to your site — itemised list: “Subdomain Takeover Scan” section); no takeover indicator found (no dangling record a third party could claim)`);
  const apiRow = !ev.api.ok ? t('⚠️ İncelenemedi', '⚠️ Nicht prüfbar', '⚠️ Not assessable') : ev.api.spec ? t(`⚠️ Herkese açık API şeması bulundu (${ev.api.spec.endpointCount} uç nokta — dökümü: “API & Swagger Keşfi” bölümü)`, `⚠️ Öffentlich zugängliches API-Schema gefunden (${ev.api.spec.endpointCount} Endpunkte — Aufschlüsselung: Abschnitt „API- & Swagger-Erkundung")`, `⚠️ Publicly accessible API schema found (${ev.api.spec.endpointCount} endpoints — breakdown: “API & Swagger Reconnaissance” section)`) : ev.api.reachable.length ? t(`⚠️ ${ev.api.reachable.length} API dokümantasyon arayüzü açık (yolları: “API & Swagger Keşfi” bölümü)`, `⚠️ ${ev.api.reachable.length} API-Dokumentationsoberflächen offen (Pfade: Abschnitt „API- & Swagger-Erkundung")`, `⚠️ ${ev.api.reachable.length} API documentation interfaces open (paths: “API & Swagger Reconnaissance” section)`) : t(`✅ ${apiTriedTotal} yol denendi (${ev.api.tried.length} sabit + ${ev.api.minedTried?.length ?? 0} site-haritası adayı, ${paScanned} sayfadan — denenen yolların TAM listesi: “API & Swagger Keşfi” bölümü); herkese açık API şeması bulunamadı (dışarıdan okunabilen API dokümanı yok)`, `✅ ${apiTriedTotal} Pfade geprüft (${ev.api.tried.length} fest + ${ev.api.minedTried?.length ?? 0} Sitemap-Kandidaten, aus ${paScanned} Seiten — VOLLSTÄNDIGE Pfadliste: Abschnitt „API- & Swagger-Erkundung"); kein öffentlich zugängliches API-Schema gefunden (kein von außen lesbares API-Dokument)`, `✅ ${apiTriedTotal} paths checked (${ev.api.tried.length} fixed + ${ev.api.minedTried?.length ?? 0} sitemap candidates, from ${paScanned} pages — FULL path list: “API & Swagger Reconnaissance” section); no publicly accessible API schema found (no externally readable API document)`);
  const cmsRow = !ev.cms.ok ? t('⚠️ İncelenemedi', '⚠️ Nicht prüfbar', '⚠️ Not assessable') : ev.cms.cms ? (ev.cms.cveTotal ? t(`⚠️ ${ev.cms.cms}${ev.cms.version ? ' ' + ev.cms.version : ''} — bilinen ${ev.cms.cveTotal} CVE eşleşti (CVE numaraları: “CMS & Bilinen CVE Taraması” bölümü)`, `⚠️ ${ev.cms.cms}${ev.cms.version ? ' ' + ev.cms.version : ''} — Übereinstimmung mit ${ev.cms.cveTotal} bekannten CVEs`, `⚠️ ${ev.cms.cms}${ev.cms.version ? ' ' + ev.cms.version : ''} — matches ${ev.cms.cveTotal} known CVEs`) : t(`✅ ${ev.cms.cms}${ev.cms.version ? ' ' + ev.cms.version : ''} tespit edildi; sürümü kapsayan bilinen yüksek CVE eşleşmedi`, `✅ ${ev.cms.cms}${ev.cms.version ? ' ' + ev.cms.version : ''} erkannt; keine die Version abdeckende bekannte hohe CVE übereinstimmend`, `✅ ${ev.cms.cms}${ev.cms.version ? ' ' + ev.cms.version : ''} detected; no known high CVE covering the version matched`)) : t(`✅ ${FP_SIGNAL_COUNT} pasif sinyal kategorisi incelendi (başlıklar, meta generator, HTML izleri, sürüm dosyaları, bilinen CMS yolları, kütüphane ipuçları — tam liste: “CMS & Bilinen CVE Taraması” bölümü); bilinen bir CMS/çatı parmak izi tespit edilmedi`, `✅ ${FP_SIGNAL_COUNT} passive Signalkategorien untersucht (Header, Meta-Generator, HTML-Spuren, Versionsdateien, bekannte CMS-Pfade, Bibliothekshinweise — vollständige Liste: Abschnitt „CMS & bekannte CVE"); kein bekannter CMS-/Framework-Fingerabdruck festgestellt`, `✅ ${FP_SIGNAL_COUNT} passive signal categories examined (headers, meta generator, HTML traces, version files, known CMS paths, library hints — full list: “CMS & known CVE” section); no known CMS/framework fingerprint detected`);
  ...
  const banRow = !(ev.bannerCves ?? []).length
    ...
    : banHits.length
      ? t(`⚠️ Banner sürümünde ${banHits.reduce((n, b) => n + b.cveTotal, 0)} bilinen CVE (${banHits.map((b) => `${b.product} ${b.version}`).join(', ')} — CVE numaraları: “Sunucu/Yazılım Banner Sürümü” bölümü)`, `⚠️ ${banHits.reduce((n, b) => n + b.cveTotal, 0)} bekannte CVE in Banner-Version (${banHits.map((b) => `${b.product} ${b.version}`).join(', ')})`, `⚠️ ${banHits.reduce((n, b) => n + b.cveTotal, 0)} known CVEs in banner version (${banHits.map((b) => `${b.product} ${b.version}`).join(', ')})`)
      : banQueried.length
        ? t(`✅ ${banQueried.length} banner sürümü NVD’ye soruldu (${banQueried.map((b) => `${b.product} ${b.version}`).join(', ')}); eşleşen bilinen CVE yok`, `✅ ${banQueried.length} Banner-Version(en) an NVD abgefragt (${banQueried.map((b) => `${b.product} ${b.version}`).join(', ')}); keine passende bekannte CVE`, `✅ ${banQueried.length} banner version(s) queried against NVD (${banQueried.map((b) => `${b.product} ${b.version}`).join(', ')}); no matching known CVE`)
  ...
  const robotsRow = minedSensExists
    ? t(`⚠️ ${minedSensExists} idari-görünümlü yol erişilebilir (yolların listesi: “API & Swagger Keşfi” bölümü; yetki testi Aktif Doğrulama kapsamı)`, `⚠️ ${minedSensExists} administrativ wirkende Pfade erreichbar (Pfadliste: Abschnitt „API- & Swagger-Erkundung"; Berechtigungsprüfung im Umfang der Aktiven Verifikation)`, `⚠️ ${minedSensExists} administrative-looking paths accessible (path list: “API & Swagger Reconnaissance” section; authorisation testing within Active Verification scope)`)
    : t(`✅ Site haritası + robots.txt Disallow’dan türetilen ${minedN} yol denendi (yalnız var/yok kontrolü — aday yolların listesi: “API & Swagger Keşfi” bölümü); hassas/idari uç bulunamadı`, `✅ ${minedN} aus Sitemap + robots.txt-Disallow abgeleitete Pfade geprüft (nur Vorhanden/Nicht-Vorhanden — Kandidatenliste: Abschnitt „API- & Swagger-Erkundung"); kein sensibler/administrativer Endpunkt gefunden`, `✅ ${minedN} paths derived from the sitemap + robots.txt Disallow tried (existence check only — candidate list: “API & Swagger Reconnaissance” section); no sensitive/administrative endpoint found`);
  ...
  const usomRow = usomHits.length
    ? `⚠️ ${usomHits.length} kamuya açık bildirim imzası gözlemlendi (sürüm doğrulanamadı — gösterge; hangi bildirimler: “USOM/SGB Bildirim Eşlemesi” bölümündeki eşleşme tablosu)`
    : `✅ ${observableCount(usomCatalog)} dış-gözlemlenebilir bildirim tarandı (hangi bildirimler olduğu — TR no + başlık + kaynak: “USOM/SGB Bildirim Eşlemesi” bölümündeki tam liste); eşleşme bulunamadı`;
  ...
  const assuranceSection = en
    ? `## POSITIVE ASSURANCE — RECONNAISSANCE METHODS CHECKED\n\n` +
      `Reconnaissance comes out clean on most healthy targets; this section also makes the “nothing found” result TRANSPARENT — it shows what was ACTUALLY checked. The **${paScanned} pages** below are the home page plus the pages derived from the sitemap and robots.txt (no page outside your own site was fetched):\n\n` +
      ...
      `**ASSESSES (passive reconnaissance — GET only, external sources):** subdomain inventory + takeover (dangling CNAME), publicly accessible API/Swagger/OpenAPI document, CMS/framework and server/software banner (Apache/nginx/PHP) fingerprint + known CVE match (NVD), EXISTENCE determination of API/administrative-looking paths derived from the sitemap + robots.txt Disallow entries — across the ${paScanned} pages defined above (home page + sitemap/robots.txt derivations).\n\n` +
    ...
    ? `## POSITIVE ZUSICHERUNG — GEPRÜFTE ERKUNDUNGSMETHODEN\n\n` +
      `Die Erkundung fällt bei den meisten gesunden Zielen sauber aus; dieser Abschnitt macht auch das Ergebnis „nichts gefunden" TRANSPARENT — er zeigt, was TATSÄCHLICH geprüft wurde. Die unten genannten **${paScanned} Seiten** sind die Startseite plus die aus Sitemap und robots.txt abgeleiteten Seiten (keine Seite außerhalb Ihrer eigenen Website wurde abgerufen):\n\n` +
      ...
      `**BEWERTET (passive Erkundung — nur GET, externe Quellen):** Subdomain-Inventar + Übernahme (dangling CNAME), öffentlich zugängliches API-/Swagger-/OpenAPI-Dokument, CMS-/Framework- und Server-/Software-Banner-Fingerabdruck (Apache/nginx/PHP) + bekannte CVE-Übereinstimmung (NVD), EXISTENZ-Feststellung aus Sitemap + robots.txt-Disallow abgeleiteter API-/administrativ wirkender Pfade — über die oben definierten ${paScanned} Seiten (Startseite + Sitemap-/robots.txt-Ableitungen).\n\n` +
    ...
    : `## POZİTİF GÜVENCE — DENENEN KEŞİF YÖNTEMLERİ\n\n` +
      `Keşif çoğu sağlıklı hedefte temiz çıkar; bu bölüm "bir şey bulunamadı" sonucunu da ŞEFFAF kılar — GERÇEKTEN ne denendiğini gösterir. Aşağıda geçen **${paScanned} sayfa**, ana sayfanız ile site haritası ve robots.txt’ten türetilen sayfalardır (kendi sitenizin dışında hiçbir sayfa çekilmemiştir):\n\n` +
