...
export async function collectEmailDnsEvidence(host: string, locale: string = 'tr'): Promise<ActiveCheckEvidence> {
  ...
  const push = (f: Partial<VFinding> & { check: string; evidence: string; severity: VFinding['severity'] }) =>
    findings.push({ inputPoint: org, vulnerable: true, technique: (en ? 'passive DNS/TXT observation' : 'pasif DNS/TXT gözlemi'), confidence: 'high', sideEffectRisk: 'none', ...f } as VFinding);

  const mail = await hasMx(org); queries++;
  const mailNuance = mail ? '' : (en ? ' (no MX on this domain — may be a non-mail-sending domain; severity at indicator level)' : ' (bu alanda MX yok — e-posta göndermeyen alan olabilir; şiddet gösterge düzeyinde)');
  ...
  if (!dmarcRec) {
    push({ check: 'dmarc_missing', severity: dl('medium'), evidence: (en ? `No DMARC record (_dmarc TXT) was **observed** for \`${org}\` — e-mail spoofing of the domain cannot be rejected by policy at the recipient (an indicator)${mailNuance}. At least \`p=quarantine\`/\`reject\` is recommended.` : `\`${org}\` için DMARC kaydı (_dmarc TXT) **gözlenmedi** — alan adına yapılan e-posta spoofing'i alıcıda politikayla reddedilemez (gösterge)${mailNuance}. En az \`p=quarantine\`/\`reject\` önerilir.`) });
  } else {
    ...
    else if (d.p === 'none') push({ check: 'dmarc_policy_weak', severity: dl('medium'), evidence: (en ? `DMARC **p=none** (${dmarcLevel}) — monitoring only; spoofing e-mails are not blocked at the recipient (an indicator)${mailNuance}. A gradual tightening \`p=quarantine\`→\`reject\` is recommended.` : `DMARC **p=none** (${dmarcLevel}) — yalnız izleme; spoofing e-postaları alıcıda engellenmiyor (gösterge)${mailNuance}. \`p=quarantine\`→\`reject\` kademeli sıkılaştırma önerilir.`) });
    else if (d.p === 'quarantine' || (d.pct !== undefined && d.pct < 100)) push({ check: 'dmarc_policy_partial', severity: 'low', evidence: (en ? `DMARC **p=${d.p ?? '?'}${d.pct !== undefined ? `, pct=${d.pct}` : ''}** (${dmarcLevel}) — partial enforcement; can be raised to \`p=reject, pct=100\` (an indicator).` : `DMARC **p=${d.p ?? '?'}${d.pct !== undefined ? `, pct=${d.pct}` : ''}** (${dmarcLevel}) — kısmi uygulama; \`p=reject, pct=100\`'e yükseltilebilir (gösterge).`) });
    if (host !== org && d.sp === 'none') push({ check: 'dmarc_subdomain_open', severity: dl('medium'), evidence: (en ? `Organization-domain DMARC **sp=none** — subdomains (including \`${host}\`) are unprotected by DMARC (an indicator)${mailNuance}.` : `Org-alan DMARC **sp=none** — alt-alanlar (\`${host}\` dahil) DMARC korumasız (gösterge)${mailNuance}.`) });
  ...
  if (!spfRec) push({ check: 'spf_missing', severity: dl('medium'), evidence: (en ? `No SPF record (v=spf1 TXT) was **observed** for \`${org}\` — authorized sending servers are undefined (an indicator)${mailNuance}.` : `\`${org}\` için SPF kaydı (v=spf1 TXT) **gözlenmedi** — yetkili gönderen sunucular tanımsız (gösterge)${mailNuance}.`) });
  else {
    const s = parseSpf(spfRec); notes.push(`SPF: \`${spfRec}\``);
    if (s.all === '+') push({ check: 'spf_permissive', severity: 'high', evidence: (en ? `SPF **+all** — ANYONE can send e-mail as this domain (fully open to spoofing). It should be \`-all\` (hardfail).` : `SPF **+all** — HERKES bu alan adına e-posta gönderebilir (spoofing'e tamamen açık). \`-all\` (hardfail) olmalı.`) });
    else if (s.all === '?') push({ check: 'spf_neutral', severity: dl('medium'), evidence: (en ? `SPF **?all** (neutral) — does not explicitly reject an unauthorized sender (an indicator)${mailNuance}. \`-all\` is recommended.` : `SPF **?all** (neutral) — yetkisiz göndereni açıkça reddetmiyor (gösterge)${mailNuance}. \`-all\` önerilir.`) });
    ...
    if (s.lookups > 10) push({ check: 'spf_too_many_lookups', severity: 'medium', evidence: (en ? `SPF top-level DNS-lookup mechanism count is **${s.lookups}** — the RFC 7208 limit is 10; exceeding it can invalidate SPF with a **PermError** (an indicator; full recursive expansion was not performed).` : `SPF üst-seviye DNS-arama mekanizması sayısı **${s.lookups}** — RFC 7208 sınırı 10; aşımda **PermError** ile SPF geçersiz kalabilir (gösterge; tam özyineli genişletme yapılmadı).`) });
  }
  ...
  if (foundSelectors.length === 0) notes.push(t('DKIM: yaygın seçicilerde (default/google/selector1…) kayıt **gözlenmedi** — kesin yokluk DEĞİL; özel seçici kullanılıyor olabilir (dürüst).', 'DKIM: Bei den gängigen Selektoren (default/google/selector1…) wurde **kein Eintrag beobachtet** — KEIN sicheres Fehlen; möglicherweise wird ein eigener Selektor verwendet (ehrlich).', 'DKIM: no record was **observed** at common selectors (default/google/selector1…) — NOT a definitive absence; a custom selector may be in use (honest).'));
    ...
    if (revoked) push({ check: 'dkim_revoked', severity: 'low', evidence: (en ? `A DKIM selector has an **empty** \`p=\` — a revoked/disabled key (an indicator).` : `Bir DKIM seçicisinde \`p=\` **boş** — iptal edilmiş/devre-dışı anahtar (gösterge).`) });
    if (weakKey) push({ check: 'dkim_weak_key', severity: 'low', evidence: (en ? `A DKIM selector's key appears to be **~1024-bit** (approx.) — 2048-bit is recommended (an indicator).` : `Bir DKIM seçicisinde anahtar **~1024-bit** görünüyor (yaklaşık) — 2048-bit önerilir (gösterge).`) });
  }
  ...
  if (!stsTxt) notes.push(t('MTA-STS: \`_mta-sts\` TXT gözlenmedi — SMTP MITM/downgrade koruması yok (düşük/olgunluk göstergesi).', 'MTA-STS: \`_mta-sts\`-TXT nicht beobachtet — kein Schutz gegen SMTP-MITM/Downgrade (niedrig/Reifegrad-Indikator).', 'MTA-STS: no \`_mta-sts\` TXT observed — no SMTP MITM/downgrade protection (a low/maturity indicator).'));
    ...
    else push({ check: 'mta_sts_weak', severity: 'low', evidence: (en ? `MTA-STS policy **mode=${mode || 'unreadable/none'}** — not \`enforce\`; no enforcing protection against SMTP MITM/downgrade (an indicator).` : `MTA-STS politikası **mode=${mode || 'okunamadı/none'}** — \`enforce\` değil; SMTP MITM/downgrade'e karşı zorlayıcı koruma yok (gösterge).`) });
  }
