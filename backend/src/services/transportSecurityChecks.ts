...
export async function collectTransportSecurityEvidence(host: string, locale: string = 'tr'): Promise<ActiveCheckEvidence> {
  ...
  for (const url of corsTargets) {
    ...
    else if (acao === '*' && acac !== 'true') notes.push(t(`CORS: \`${ep}\` \`Access-Control-Allow-Origin: *\` (credentials yok) — public kaynak için yaygın/kabul edilebilir (bilgilendirici).`, `CORS: \`${ep}\` \`Access-Control-Allow-Origin: *\` (keine Credentials) — für öffentliche Ressourcen üblich/akzeptabel (informativ).`));
  }
  ...
  if (corsTargets.length === 0) notes.push(t('CORS: test edilebilir gerçek uç bulunamadı — CORS alt-kontrolü bu hedefte kapsam dışı.', 'CORS: Kein testbarer echter Endpunkt gefunden — die CORS-Teilprüfung ist für dieses Ziel außerhalb des Geltungsbereichs.'));
  ...
  if (anyHandshake) {
    notes.push(t(`TLS: desteklenen protokoller — **${supported.join(', ') || '(tespit edilemedi)'}**${supported.includes('TLS 1.2') || supported.includes('TLS 1.3') ? ' (TLS 1.2/1.3 mevcut — olumlu)' : ''}.`, `TLS: unterstützte Protokolle — **${supported.join(', ') || '(nicht ermittelbar)'}**${supported.includes('TLS 1.2') || supported.includes('TLS 1.3') ? ' (TLS 1.2/1.3 vorhanden — positiv)' : ''}.`));
    ...
    if (cert && Object.keys(cert).length) {
      ...
      else notes.push(t('TLS sertifikası (süre/hostname/zincir) handshake sırasında geçerli görünüyor — belirgin sertifika sorunu gözlenmedi (olumlu).', 'Das TLS-Zertifikat (Gültigkeit/Hostname/Kette) erscheint beim Handshake gültig — kein auffälliges Zertifikatsproblem beobachtet (positiv).'));
    }
  ...
  if (hr) {
    ...
    else { const m = hsts.match(/max-age\s*=\s*(\d+)/i); const age = m ? parseInt(m[1], 10) : 0; if (age < 15552000) findings.push({ check: 'hsts_weak', inputPoint: new URL(origin).host, vulnerable: true, technique: 'HSTS max-age yeterlilik gözlemi', evidence: `HSTS mevcut ama \`max-age=${age}\` **kısa** (< 180 gün) — koruma penceresi dar. \`max-age≥15768000\` (1 yıl) + \`includeSubDomains\` önerilir (gösterge).`, confidence: 'high', severity: 'low', sideEffectRisk: 'none' }); else notes.push(t(`HSTS mevcut (max-age=${age}${/includesubdomains/i.test(hsts) ? '; includeSubDomains' : ''}${/preload/i.test(hsts) ? '; preload' : ''}) — olumlu.`, `HSTS vorhanden (max-age=${age}${/includesubdomains/i.test(hsts) ? '; includeSubDomains' : ''}${/preload/i.test(hsts) ? '; preload' : ''}) — positiv.`)); }
  ...
  } else notes.push(t('Güvenlik başlığı derinliği: HTTPS ana sayfa başlıkları okunamadı — bu alt-kontrol sınırlı.', 'Sicherheits-Header-Tiefe: Die HTTPS-Startseiten-Header konnten nicht gelesen werden — diese Teilprüfung ist eingeschränkt.'));
...
export async function collectSubdomainTakeoverEvidence(host: string, locale: string = 'tr'): Promise<ActiveCheckEvidence> {
  ...
  if (sub.dataSource === 'unavailable') return { ok: true, pagesScanned: 0, inputsFound: 0, probesSent: 1, findings, stopped: t('CT veri kaynağına ulaşılamadı', 'CT-Datenquelle nicht erreichbar'), notes: [t('Subdomain takeover: Certificate Transparency kaynaklarına (crt.sh/certSpotter) ulaşılamadı — **incelenemedi** (temiz değil).', 'Subdomain-Übernahme: Die Certificate-Transparency-Quellen (crt.sh/certSpotter) waren nicht erreichbar — **konnte nicht geprüft werden** (nicht sauber).')] };
