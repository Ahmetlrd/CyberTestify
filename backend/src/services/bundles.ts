...
export const COMBO_BUNDLES: ComboBundle[] = [
  ...
  },
  {
    ...
    description:
      'Kimlik doğrulamalı (login’li) tarama + **yapay zekâ destekli** analiz bir arada. Sağladığınız bir TEST hesabıyla login sonrası çerez/oturum/yetki, authenticated enjeksiyon/IDOR, yetki yükseltme ve çok-adımlı iş mantığı göstergeleri incelenir. Deterministik güvenlik kontrolleri + iki kontrolde yapay zekâ destekli analiz (tek LLM danışma çağrısı; otonom/sınırsız pentest DEĞİLDİR). Aktif-hafif sınırlar korunur; gerçek veri/hesap değişikliği ve ödeme tamamlama **kod seviyesinde engellidir**. ' +
      ...
      '**Kapsam:** Cross-account (başka bir kullanıcının verisine erişim) IDOR bu sürümün kapsamı dışındadır. Yapay zekâ destekli analiz katmanı bazı hedeflerde/durumlarda uygulanabilir bir gösterge bulamayabilir; bu durumda sonuçlar deterministik authenticated kontrollerle sınırlı kalır — bu normal bir davranıştır ve rapor bunu şeffaf gösterir.',
    descriptionEn:
      'Authenticated (logged-in) scan + **AI-assisted** analysis. Using a TEST account you provide, it reviews post-login cookie/session/authorization, authenticated injection/IDOR, privilege escalation and multi-step business-logic indicators. Deterministic security checks + AI-assisted analysis on two checks (a single LLM advisory call; NOT an autonomous/unlimited pentest). Active-light limits preserved; real data/account changes and payment completion are BLOCKED at the code level. ' +
      ...
      'Scope: cross-account IDOR (accessing another user’s data) is out of scope in this version. The AI-assisted analysis layer may not find an applicable indicator on some targets; in that case results are limited to the deterministic authenticated checks — this is normal and the report shows it transparently.',
    category: 'active-light',
    discountPct: 20,
