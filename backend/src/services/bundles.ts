...
export const COMBO_BUNDLES: ComboBundle[] = [
  ...
  },
  {
    ...
    description:
      ...
      '**Önemli:** Bu paket güvenlik nedeniyle sipariş sonrası kısa bir manuel inceleme sürecinden geçer; tarama genellikle **24 saat içinde** başlar. Bir **TEST hesabı** (2FA’sız, sınırlı yetkili, ana/üretim hesabınız DEĞİL, tek-kullanımlık) gerekir; kimlik bilgileriniz **şifreli/geçici** saklanır ve tarama sonrası silinir. Yetkilendirme beyanı gerekir. ' +
      '**Kapsam:** Cross-account (başka bir kullanıcının verisine erişim) IDOR bu sürümün kapsamı dışındadır. Sınırlı/kontrollü otonom ajan katmanı bazı hedeflerde/durumlarda analizi tamamlayamayabilir; bu durumda sonuçlar deterministik authenticated kontrollerle sınırlı kalır — bu normal bir davranıştır ve rapor bunu şeffaf gösterir.',
    descriptionEn:
      ...
      'Note: for safety this package goes through a short manual review after ordering; scanning usually starts within 24 hours. Requires a dedicated TEST account (no 2FA, least-privilege, NOT your production account, single-use); credentials are stored encrypted/temporarily and deleted after the scan. Requires an authorization declaration. ' +
      'Scope: cross-account IDOR (accessing another user’s data) is out of scope in this version. The limited/controlled autonomous agent layer may not complete its analysis on some targets; in that case results are limited to the deterministic authenticated checks — this is normal and the report shows it transparently.',
    category: 'active-light',
    discountPct: 20,
