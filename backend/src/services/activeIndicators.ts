...
export async function collectActiveIndicatorsEvidence(host: string, session?: AuthSession, locale: string = 'tr'): Promise<ActiveCheckEvidence> {
  ...
  if (getParams.length === 0 && surf.uploadForms.length === 0) {
    return { ok: true, pagesScanned: surf.pagesScanned, inputsFound: 0, probesSent: 1, findings, stopped: null,
      notes: [`${authed ? t('Login-sonrası', 'Nach dem Login', 'Post-login') : t('Gözlemlenebilir', 'Beobachtbare', 'Observable')}${t(' enjekte-edilebilir bir yüzey (GET parametresi / yükleme formu) bu hedefte bulunamadı — güvenli aktif göstergeler **kapsam dışıdır** (iyi-yapılandırılmış/SPA sitelerde beklenen sonuç).', ' injizierbare Oberfläche (GET-Parameter / Upload-Formular) wurde bei diesem Ziel nicht gefunden — die sicheren aktiven Indikatoren sind **außerhalb des Geltungsbereichs** (bei gut konfigurierten/SPA-Websites das erwartete Ergebnis).', ' injectable surface (GET parameter / upload form) was found on this target — the safe active indicators are **out of scope** (the expected result on well-configured/SPA sites).')}`] };
  }
  ...
  for (const ip of getParams.filter((p) => REDIRECT_PARAM_RE.test(p.param)).slice(0, 8)) {
    ...
    if ((r.status >= 300 && r.status < 400 && reflectsCanary(loc)) || reflectsCanary(metaRefresh)) {
      findings.push({ check: 'open_redirect', inputPoint: epLabel(ip), vulnerable: true, technique: `${t('açık yönlendirme (open redirect) — zararsız kanarya gözlemi (redirect TAKİP EDİLMEDİ)', 'Offene Weiterleitung (open redirect) — harmlose Kanarienvogel-Beobachtung (Weiterleitung NICHT gefolgt)', 'open redirect — harmless canary observation (redirect NOT followed)')}${authed ? ' (authenticated)' : ''}`, evidence: t(`\`${ip.param}\` parametresine verilen **harici kanarya** URL'i yanıtın \`Location\`/meta-refresh hedefine **yansıdı** — açık yönlendirme göstergesi (phishing/oturum-token sızıntısı yüzeyi). Yönlendirme hedefleri sunucuda allowlist ile sınırlanmalı. Yönlendirme TAKİP EDİLMEDİ.`, `Die dem Parameter \`${ip.param}\` übergebene **externe Kanarienvogel**-URL wurde im \`Location\`-/meta-refresh-Ziel der Antwort **widergespiegelt** — Indikator für eine offene Weiterleitung (Angriffsfläche für Phishing/Sitzungs-Token-Leck). Weiterleitungsziele sollten serverseitig per Allowlist eingeschränkt werden. Der Weiterleitung wurde NICHT gefolgt.`, `The **external canary** URL supplied to the \`${ip.param}\` parameter was **reflected** in the response's \`Location\`/meta-refresh target — an open-redirect indicator (phishing/session-token-leak surface). Redirect targets should be restricted server-side with an allowlist. The redirect was NOT followed.`), confidence: 'high', severity: 'medium', sideEffectRisk: 'none' });
    }
  }
  ...
  for (const ip of getParams.slice(0, 6)) {
    ...
    if (both) findings.push({ check: 'hpp', inputPoint: epLabel(ip), vulnerable: true, technique: `${t('HTTP parametre kirliliği (HPP) — tekrarlı parametre işleniş gözlemi (veri gönderilmedi)', 'HTTP-Parameter-Pollution (HPP) — Beobachtung der Verarbeitung wiederholter Parameter (keine Daten gesendet)', 'HTTP parameter pollution (HPP) — observation of repeated-parameter processing (no data sent)')}${authed ? ' (authenticated)' : ''}`, evidence: t(`\`${ip.param}\` iki kez gönderildiğinde (\`?${ip.param}=A&${ip.param}=B\`) sunucu **her iki değeri birleştirerek** işledi — HTTP Parameter Pollution göstergesi (filtre atlatma/mantık sapması yüzeyi). Parametreler tek-değere normalize edilmeli. Salt gözlem.`, `Als \`${ip.param}\` zweimal gesendet wurde (\`?${ip.param}=A&${ip.param}=B\`), verarbeitete der Server **beide Werte zusammengefügt** — Indikator für HTTP Parameter Pollution (Angriffsfläche für Filterumgehung/Logikabweichung). Parameter sollten auf einen einzigen Wert normalisiert werden. Reine Beobachtung.`, `When \`${ip.param}\` was sent twice (\`?${ip.param}=A&${ip.param}=B\`), the server processed **both values concatenated** — an HTTP Parameter Pollution indicator (filter-bypass/logic-deviation surface). Parameters should be normalised to a single value. Observation only.`), confidence: 'medium', severity: 'low', sideEffectRisk: 'none' });
    else if (onlyB) notes.push(t(`HPP: \`${ip.param}\` tekrarında son-değer (last-wins) ayrıştırma gözlendi — tek başına zafiyet değil, bilgilendirici.`, `HPP: Bei der Wiederholung von \`${ip.param}\` wurde eine Last-Wins-Verarbeitung (letzter Wert) beobachtet — allein keine Schwachstelle, informativ.`, `HPP: on repeating \`${ip.param}\`, last-wins parsing was observed — not a vulnerability on its own, informational.`));
  }
  ...
  notes.push(`${t('Boolean-tabanlı SQL Enjeksiyonu **', 'Boolean-basierte SQL-Injection wird im Abschnitt **', 'Boolean-based SQL Injection is evaluated in the **')}${authed ? t('Authenticated Enjeksiyon (SQLi/XSS)', 'Authentifizierte Injektion (SQLi/XSS)', 'Authenticated Injection (SQLi/XSS)') : t('Enjeksiyon (SQLi/XSS) Doğrulama', 'Injektion (SQLi/XSS)-Verifizierung', 'Injection (SQLi/XSS) Verification')}${t('** bölümünde katı yanlış-pozitif kalkanıyla değerlendirilir (burada tekrar edilmez).', '** mit einem strengen Falsch-Positiv-Schutz bewertet (hier nicht wiederholt).', '** section with a strict false-positive shield (not repeated here).')}`);
  if (surf.uploadForms.length) {
    notes.push(authed
      ? t(`Login-sonrası **${surf.uploadForms.length}** dosya yükleme noktası gözlendi; **gerçek dosya YÜKLENMEDİ** (konfig gözlemi — güvenli). Sunucu-taraflı tip/boyut doğrulaması manuel test gerektirir.`, `Nach dem Login wurden **${surf.uploadForms.length}** Datei-Upload-Punkte beobachtet; **es wurde KEINE echte Datei hochgeladen** (Konfigurationsbeobachtung — sicher). Die serverseitige Typ-/Größenvalidierung erfordert einen manuellen Test.`, `Post-login, **${surf.uploadForms.length}** file-upload points were observed; **NO real file was uploaded** (configuration observation — safe). Server-side type/size validation requires a manual test.`)
      : t(`Dosya yükleme yüzeyi (**${surf.uploadForms.length}** form) **Dosya Yükleme Doğrulama** bölümünde değerlendirilir (gerçek dosya yüklenmeden).`, `Die Datei-Upload-Oberfläche (**${surf.uploadForms.length}** Formular(e)) wird im Abschnitt **Datei-Upload-Verifizierung** bewertet (ohne dass eine echte Datei hochgeladen wird).`, `The file-upload surface (**${surf.uploadForms.length}** form(s)) is evaluated in the **File Upload Verification** section (without uploading a real file).`));
  }

  notes.push(`${t('Denenen: **', 'Durchgeführt: **', 'Performed: **')}${probes}${t('** güvenli read-only prob', '** sichere schreibgeschützte Probes', '** safe read-only probes')}${authed ? t(' (login-sonrası oturumla)', ' (mit der Sitzung nach dem Login)', ' (with the post-login session)') : ''}${t(' (yalnız GET; yükleme/komut/time-based YOK). LFI yalnız imza — içerik REDAKTE; open-redirect kanaryası TAKİP EDİLMEDİ; SSTI yalnız aritmetik; HPP salt gözlem. Test edilen GET parametresi: **', ' (nur GET; kein Upload/Befehl/time-based). LFI nur Signatur — Inhalt REDIGIERT; der Open-Redirect-Kanarienvogel wurde NICHT verfolgt; SSTI nur Arithmetik; HPP reine Beobachtung. Getestete GET-Parameter: **', ' (GET only; no upload/command/time-based). LFI signature only — content REDACTED; the open-redirect canary was NOT followed; SSTI arithmetic only; HPP observation only. GET parameters tested: **')}${getParams.length}**${authed ? t(' (Faz 7 SPA-keşif + auth-kapılı uçlar dâhil)', ' (inkl. Phase-7-SPA-Erkundung + auth-geschützte Endpunkte)', ' (incl. Phase-7 SPA discovery + auth-gated endpoints)') : ''}.`);
