...
export async function collectApiSecurityEvidence(host: string, session: AuthSession, locale: string = 'tr'): Promise<ActiveCheckEvidence> {
  ...
  for (const api of realApi) {
    ...
    if (sens) findings.push({
      check: 'excessive_data_exposure', inputPoint: api.path, vulnerable: true, technique: (en ? 'excessive data exposure / BOPLA (API3) — response field observation' : de ? 'übermäßige Datenoffenlegung / BOPLA (API3) — Antwortfeld-Beobachtung' : 'aşırı veri ifşası / BOPLA (API3) — yanıt alan gözlemi'),
      evidence: (en ? `The API response (\`${api.path}\`) contains a **sensitive field** (\`${sens[1]}\`) — value REDACTED. The response sent to the client exposes more/sensitive data than needed (password-hash/secret/token etc.). A field allowlist (response DTO) should be applied server-side.` : de ? `Die API-Antwort (\`${api.path}\`) enthält ein **sensibles Feld** (\`${sens[1]}\`) — Wert REDIGIERT. Die an den Client gesendete Antwort legt mehr/sensiblere Daten offen als nötig (Passwort-Hash/Geheimnis/Token usw.). Serverseitig sollte eine Feld-Allowlist (Antwort-DTO) angewendet werden.` : `API yanıtı (\`${api.path}\`) **hassas alan** içeriyor (\`${sens[1]}\`) — değer REDAKTE. İstemciye gönderilen yanıt gerekenden fazla/hassas veri ifşa ediyor (parola-hash/sır/token vb.). Sunucuda alan-allowlist (response DTO) uygulanmalı.`),
      confidence: 'high', severity: 'high', sideEffectRisk: 'none',
    });
    else if (over) findings.push({
      check: 'excessive_data_exposure', inputPoint: api.path, vulnerable: true, technique: (en ? 'excessive data exposure / BOPLA (API3) — response field observation' : de ? 'übermäßige Datenoffenlegung / BOPLA (API3) — Antwortfeld-Beobachtung' : 'aşırı veri ifşası / BOPLA (API3) — yanıt alan gözlemi'),
      evidence: (en ? `The API response (\`${api.path}\`) contains internal/excessive fields the UI does not need (\`${over[1]}\`) — internal ID/role/status fields leak to the client (an indicator). Restrict the response to only the needed fields (allowlist DTO).` : de ? `Die API-Antwort (\`${api.path}\`) enthält interne/übermäßige Felder, die die UI nicht benötigt (\`${over[1]}\`) — interne ID-/Rollen-/Status-Felder gelangen an den Client (ein Indikator). Beschränken Sie die Antwort auf nur die benötigten Felder (Allowlist-DTO).` : `API yanıtı (\`${api.path}\`) UI'nin ihtiyaç duymadığı iç/aşırı alan içeriyor (\`${over[1]}\`) — iç ID/rol/durum alanları istemciye sızıyor (gösterge). Yanıtı yalnız gereken alanlarla sınırlayın (allowlist DTO).`),
  ...
  if (realApi.length) {
    ...
    if (limited) notes.push(t('API rate-limit gözlemlendi (429 / rate-limit başlığı) — olumlu.', 'API-Rate-Limit beobachtet (429 / Rate-Limit-Header) — positiv.', 'API rate-limit observed (429 / rate-limit header) — positive.'));
      ...
      check: 'no_rate_limit', inputPoint: new URL(realApi[0].path, origin).pathname, vulnerable: true, technique: (en ? 'API rate-limit / unrestricted consumption observation (modest burst — not DoS)' : de ? 'API-Rate-Limit / unbeschränkte-Nutzung-Beobachtung (moderater Burst — kein DoS)' : 'API rate-limit / kısıtlanmamış tüketim gözlemi (modest burst — DoS değil)'),
      evidence: (en ? `After ${times.length} consecutive requests to an API endpoint (\`${new URL(realApi[0].path, origin).pathname}\`), **no 429 or rate-limit header was observed** — an unrestricted resource consumption (API4) indicator (may be exposed to brute-force/scraping/DoS). The target was not exhausted (modest burst). Rate-limit + quota are recommended.` : de ? `Nach ${times.length} aufeinanderfolgenden Anfragen an einen API-Endpunkt (\`${new URL(realApi[0].path, origin).pathname}\`) wurde **kein 429- oder Rate-Limit-Header beobachtet** — ein Indikator für unbeschränkten Ressourcenverbrauch (API4) (kann für Brute-Force/Scraping/DoS anfällig sein). Das Ziel wurde nicht erschöpft (moderater Burst). Rate-Limit + Kontingent werden empfohlen.` : `Bir API ucuna (\`${new URL(realApi[0].path, origin).pathname}\`) art arda ${times.length} istek sonrası **429 veya rate-limit başlığı gözlenmedi** — kısıtlanmamış kaynak tüketimi (API4) göstergesi (brute-force/scraping/DoS'a açık olabilir). Hedef yorulmadı (modest burst). Rate-limit + kota önerilir.`),
      confidence: 'medium', severity: 'medium', sideEffectRisk: 'none',
    });
  }
  ...
  if (versionsSeen.length >= 2) findings.push({
    check: 'shadow_api_version', inputPoint: versionsSeen.join(', '), vulnerable: true, technique: (en ? 'shadow / deprecated API version observation (safe GET)' : de ? 'Shadow-/veraltete-API-Version-Beobachtung (sicheres GET)' : 'shadow / deprecated API sürümü gözlemi (güvenli GET)'),
    evidence: (en ? `Multiple API version endpoints are accessible at once (\`${versionsSeen.join('`, `')}\`) — old/shadow versions may remain unpatched and widen the attack surface (API9). Disable/remove unused versions. An indicator.` : de ? `Mehrere API-Versions-Endpunkte sind gleichzeitig erreichbar (\`${versionsSeen.join('`, `')}\`) — alte/Shadow-Versionen bleiben möglicherweise ungepatcht und vergrößern die Angriffsfläche (API9). Deaktivieren/entfernen Sie ungenutzte Versionen. Ein Indikator.` : `Birden fazla API sürüm ucu aynı anda erişilebilir (\`${versionsSeen.join('`, `')}\`) — eski/gölge sürümler yamasız kalıp saldırı yüzeyini genişletebilir (API9). Kullanılmayan sürümleri kapatın/kaldırın. Gösterge.`),
    confidence: 'medium', severity: 'medium', sideEffectRisk: 'none',
  });
  ...
  if (graphqlUrl) {
    ...
    if (r && r.status < 400 && /"__schema"|"types"\s*:\s*\[/.test(r.text)) findings.push({
      check: 'graphql_introspection', inputPoint: new URL(graphqlUrl).pathname, vulnerable: true, technique: (en ? 'GraphQL introspection exposure (read-only query; NO mutation)' : de ? 'GraphQL-Introspection-Offenlegung (read-only-Abfrage; KEINE Mutation)' : 'GraphQL introspection açıklığı (read-only sorgu; mutasyon YOK)'),
      evidence: (en ? `The GraphQL endpoint (\`${new URL(graphqlUrl).pathname}\`) has **introspection enabled** — the entire schema (types/fields/mutations) is exposed externally, mapping the attack surface. Introspection should be disabled in production. (Only the schema was queried; no data was modified.)` : de ? `Der GraphQL-Endpunkt (\`${new URL(graphqlUrl).pathname}\`) hat **Introspection aktiviert** — das gesamte Schema (Typen/Felder/Mutationen) wird extern offengelegt und bildet die Angriffsfläche ab. Introspection sollte in der Produktion deaktiviert werden. (Nur das Schema wurde abgefragt; keine Daten wurden verändert.)` : `GraphQL ucu (\`${new URL(graphqlUrl).pathname}\`) **introspection açık** — tüm şema (tipler/alanlar/mutasyonlar) dışarıya ifşa oluyor, saldırı yüzeyini haritalar. Üretimde introspection kapatılmalı. (Yalnız şema sorgulandı; veri değiştirilmedi.)`),
