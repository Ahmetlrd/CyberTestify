...
function domFormPrivObservation(dom: { url: string; fields: string[]; interesting: string[] }, locale: string = 'tr'): VFinding | null {
  ...
  return {
    ...
    evidence: t(`Kayıt/profil formunda istemciye açık bir yetki alanı gözlemlendi: \`${priv.join('`, `')}\`. Sunucu bu alanı yok saymıyorsa mass-assignment ile yetki yükseltme riski. **Form SUBMIT EDİLMEDİ** — bu salt-okunur bir DOM gözlemidir; kesin doğrulama manuel test gerektirir.`, `In einem Registrierungs-/Profilformular wurde ein clientseitig offengelegtes Berechtigungsfeld beobachtet: \`${priv.join('`, `')}\`. Wenn der Server dieses Feld nicht ignoriert, besteht die Gefahr einer Rechteausweitung per Mass-Assignment. **Das Formular wurde NICHT abgesendet** — dies ist eine schreibgeschützte DOM-Beobachtung; eine eindeutige Bestätigung erfordert einen manuellen Test.`, `A client-exposed privilege field was observed in a registration/profile form: \`${priv.join('`, `')}\`. If the server does not ignore this field, there is a risk of privilege escalation via mass-assignment. **The form was NOT SUBMITTED** — this is a read-only DOM observation; definitive confirmation requires a manual test.`),
...
export async function collectPrivilegeEscalationEvidence(host: string, session: AuthSession, locale: string = 'tr'): Promise<ActiveCheckEvidence> {
  ...
  if (surf.massAssignForm) {
    ...
    if (forb) notes.push(t(`Mass-assignment adayı form (${p}) YASAK türe girdiğinden gerçek POST'tan hariç tutuldu: ${forb}. Bu kontrol için güvenle test edilebilir bir yetki formu değildir.`, `Das Mass-Assignment-Kandidatenformular (${p}) wurde vom tatsächlichen POST ausgeschlossen, da es zu einem VERBOTENEN Typ gehört: ${forb}. Für diese Prüfung ist es kein sicher testbares Berechtigungsformular.`, `The mass-assignment candidate form (${p}) was excluded from the real POST because it is a FORBIDDEN type: ${forb}. It is not a privilege form that can be safely tested for this check.`));
    else if (cat === 'login' || cat === 'search') notes.push(t(`Aday form (${p}, ${cat}) yetki-yükseltme/over-posting hedefi değildir — atlandı.`, `Das Kandidatenformular (${p}, ${cat}) ist kein Ziel für Rechteausweitung/Over-Posting — übersprungen.`, `The candidate form (${p}, ${cat}) is not a target for privilege-escalation/over-posting — skipped.`));
  ...
  if (!detProbed) {
    notes.push(t('Yetki-yükseltme için GÜVENLE test edilebilir (YASAK olmayan; kayıt/profil/ayar tipi) bir form/uç bu hedefte bulunamadı — bu kontrol **İncelenemedi** (deterministik prob için uygun authenticated yüzey yok). Not: bazı açıklar (API’de gizli `role` kabulü vb.) UI/DOM’da görünmez; kesin sonuç manuel test gerektirir.', 'Ein für Rechteausweitung SICHER testbares Formular/Endpunkt (nicht verboten; Typ Registrierung/Profil/Einstellungen) wurde bei diesem Ziel nicht gefunden — diese Prüfung wurde **nicht durchgeführt** (keine geeignete authentifizierte Oberfläche für den deterministischen Prob). Hinweis: Einige Schwachstellen (z. B. verstecktes Akzeptieren von `role` in der API) sind in UI/DOM nicht sichtbar; ein eindeutiges Ergebnis erfordert einen manuellen Test.', 'A SAFELY testable form/endpoint for privilege escalation (non-forbidden; registration/profile/settings type) was not found on this target — this check was **Not assessable** (no suitable authenticated surface for the deterministic probe). Note: some vulnerabilities (e.g. hidden acceptance of `role` in the API) are not visible in the UI/DOM; a definitive result requires a manual test.'));
  } else if (!findings.length) {
    notes.push(t('Keşfedilen authenticated form(lar)a gözlemsel mass-assignment probu (`role/isAdmin` ek alan) gönderildi; sunucu açık bir reddetme ile karşıladı — yetki-yükseltme göstergesi **bulunamadı** (deterministik sonuç — advisory değil, gerçek gözlem).', 'An das/die entdeckte(n) authentifizierte(n) Formular(e) wurde ein beobachtender Mass-Assignment-Prob (Zusatzfeld `role/isAdmin`) gesendet; der Server antwortete mit einer ausdrücklichen Ablehnung — es wurde **kein** Indikator für Rechteausweitung gefunden (deterministisches Ergebnis — keine Advisory, sondern eine tatsächliche Beobachtung).', 'An observational mass-assignment probe (`role/isAdmin` extra field) was sent to the discovered authenticated form(s); the server responded with an explicit rejection — **no** privilege-escalation indicator was found (a deterministic result — not advisory, a real observation).'));
  }
  if (scenarios !== null && agentStatus === 'analyzed') notes.push(t('AI advisory bu yüzeyi ayrıca analiz etti (yardımcı bağlam; nihai karar deterministik prob/gözleme dayanır — advisory tek başına kanıt sunulmaz).', 'Die KI-Advisory hat diese Oberfläche zusätzlich analysiert (unterstützender Kontext; die endgültige Bewertung beruht auf dem deterministischen Prob/der Beobachtung — die Advisory allein wird nicht als Nachweis vorgelegt).', 'The AI advisory additionally analysed this surface (supporting context; the final decision is based on the deterministic probe/observation — the advisory alone is not presented as evidence).'));
...
export async function collectMultiStepBusinessLogicEvidence(host: string, session: AuthSession, locale: string = 'tr'): Promise<ActiveCheckEvidence> {
  ...
  for (const url of targets) {
    ...
    if (r && r.status === 200 && !/oturum|login|giriş yap|unauthorized|403|yetkisiz/i.test(r.text.slice(0, 2000))) {
      findings.push({ check: 'business_logic_multistep', inputPoint: new URL(url).pathname, vulnerable: true, technique: 'observation (step-skip, GET only)', evidence: t(`Bir "başarılı/onay" adımı sayfası (${new URL(url).pathname}) ön koşul olmadan doğrudan GET ile erişilebilir göründü — çok-adımlı iş mantığı (adım-atlama) göstergesi; kesin doğrulama manuel test gerektirir (ödeme TAMAMLANMADI).`, `Eine „Erfolgs-/Bestätigungs"-Schrittseite (${new URL(url).pathname}) schien ohne Voraussetzung direkt per GET erreichbar zu sein — Indikator für mehrstufige Geschäftslogik (Schrittüberspringen); eine eindeutige Bestätigung erfordert einen manuellen Test (Zahlung NICHT abgeschlossen).`, `A "success/confirmation" step page (${new URL(url).pathname}) appeared to be directly reachable via GET without a precondition — a multi-step business-logic (step-skipping) indicator; definitive confirmation requires a manual test (payment NOT completed).`), confidence: 'low', severity: 'low', sideEffectRisk: 'none' });
    }
  }

  if (ctx.stopped) notes.push(ctx.stopped);
  if (!findings.length && agentStatus === 'analyzed') notes.push(t('Yapay zekâ destekli advisory bu yüzeyi analiz etti; uygulanabilir bir çok-adımlı iş-mantığı vektörü tespit edilmedi (temiz sonuç — uydurma bulgu yok).', 'Die KI-gestützte Advisory hat diese Oberfläche analysiert; es wurde kein ausnutzbarer mehrstufiger Geschäftslogik-Vektor festgestellt (sauberes Ergebnis — kein erfundener Befund).', 'The AI-assisted advisory analysed this surface; no exploitable multi-step business-logic vector was detected (a clean result — no fabricated finding).'));
